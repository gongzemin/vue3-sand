...
function standardSpacing(style) {
  const changedLayers = style.layers.map((layer) => {
    ...
    return {
      ...layer,
      layout: { ...layer.layout, 'text-letter-spacing': spacing },
    };
  });

  return { ...style, layers: changedLayers };
}
...
function isNameFunctionField(isLangField, property) {
  return property.stops && property.stops.filter((stop) => isLangField.test(stop[1])).length > 0;
}
...
function adaptPropertyLanguage(isLangField, property, languageFieldName) {
  ...
  if (isNameFunctionField(isLangField, property)) {
    const newStops = property.stops.map((stop) => {
      if (isLangField.test(stop[1])) {
        return [stop[0], languageFieldName];
      }
      return stop;
    });
    return { ...property, stops: newStops };
  }
  return property;
}
...
function changeLayerTextProperty(isLangField, layer, languageFieldName, excludedLayerIds) {
  if (layer.layout && layer.layout['text-field'] && excludedLayerIds.indexOf(layer.id) === -1) {
    return {
      ...layer,
      layout: { ...layer.layout, 'text-field': adaptPropertyLanguage(isLangField, layer.layout['text-field'], languageFieldName) },
    };
  }
  return layer;
}
...
MapboxLanguage.prototype.setLanguage = function (style, language) {
  ...
  const languageStyle = { ...style, layers: changedLayers };
